...
public class GameEngineTest {
	...
	public GameEngineTest(){
		try{
			...
			this.stats = testEngine.simulate(brainArray[0], brainArray[1], testWorld);
		} catch (IOEvent e) {
			fail(e.getMessage());
	...
	public void testGetWinner(){ //this also tests the simulate competition test
		assertEquals(1,this.stats.getWinner());
	}
	
	@Test
	public void testGetRemainingFood(){
		assertEquals(0,this.stats.getFoodInBlackAnthill()); //there should be no food left in the dummy ants hill
		assertTrue(this.stats.getFoodInRedAnthill() > 0); //there should be more than 0 food in this ant hill
	}
}
